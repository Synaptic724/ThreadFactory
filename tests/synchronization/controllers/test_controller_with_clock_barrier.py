...
class ControllerClockBarrierTests(unittest.TestCase):
    ...
    def test_barrier_auto_registers_with_controller(self):
        ...
        barrier = ClockBarrier(threshold=1, timeout=0.1, controller=ctrl)
...
class ControllerClockBarrierExtraTests(unittest.TestCase):
    ...
    def test_barrier_reset_reuse(self):
        ...
        # Round 1
        t1, t2 = threading.Thread(target=wait_once), threading.Thread(target=wait_once)
        t1.start(); t2.start(); t1.join(); t2.join()

        barrier.reset()

        # Round 2
        t3, t4 = threading.Thread(target=wait_once), threading.Thread(target=wait_once)
    ...
    def test_dispose_mid_wait_broken_event(self):
        ...
        # FIX: The dispose method correctly breaks the barrier, which sends a
        # "BARRIER_BROKEN" event. The test should expect this.
        self.assertEqual(len(rec.events), 1)
        obj_id, event_type, _ = rec.events[0]
        self.assertEqual(obj_id, barrier.id)
        self.assertEqual(event_type, "BARRIER_BROKEN")
    ...
    def test_wait_starting_and_terminal_event_cleanup(self):
        ...
        ctrl.notify(obj.id, "RESET_BY_CONTROLLER")
