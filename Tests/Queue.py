...
class TestConcurrentQueue(unittest.TestCase):
    ...
    def test_atomic_update_swap(self):
        """
        Test atomic_update and atomic_swap for random-access modifications
        in an otherwise FIFO structure (unusual but allowed).
        """
        q = ConcurrentQueue(["a", "b", "c"])
        # update "b" -> "B"
        q.atomic_update(1, lambda old: old.upper())
        self.assertEqual(list(q), ["a", "B", "c"])

        with self.assertRaises(IndexError):
            q.atomic_update(99, lambda x: x)

        # swap "a" and "c"
        q.atomic_swap(0, 2)
        self.assertEqual(list(q), ["c", "B", "a"])
