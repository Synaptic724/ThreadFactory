...
class CommandCenter(IDisposable):
    ...
    def _register_agent(self, thread: threading.Thread, factory_id: Optional[str] = None,
        ...
        """
        Registers a thread as an ActivatedAgent, assigns it an identity profile,
        and stores it in the active agent registry.

        This method transforms the given thread into a managed agent by:
        - Wrapping it in an `ActivatedAgent` container.
        ...
        Args:
            ...
            factory_id (Optional[str]): Optional unique ID to assign to the agent. If not provided,
                                        a ULID will be generated by the ActivatedAgent constructor.
    ...
    def _assign_profile(self, agent: ActivatedAgent, profile_key: Optional[str] = None) -> None:
        ...
        Args:
            agent (ActivatedAgent): The agent to bind a profile to.
