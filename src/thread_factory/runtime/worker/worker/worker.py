...
class Worker(threading.Thread, IDisposable):
    ...
    def __init__(self, group: Optional[threading.ThreadGroup] = None,
        ...
        """
        Initializes a new Worker thread instance.
        ...
        Args:
            ...
            target (Optional[Callable]): The callable object to be run by the thread's
                `run()` method. Defaults to None, in which case `Worker.run()` is used.
        ...
        if self._signal_controller:
            try:
                ...
                self._logger.debug(f"Worker '{self.id}' auto-registered with SignalController.")
            except Exception as e:
                self._logger.warning(
                    f"Failed to auto-register Worker '{self.id}' with SignalController: {e}", exc_info=True)

    def dispose(self):
        """
        Disposes of the Worker's resources, halts the thread, and unregisters itself.
        ...
        with self._lock:
            ...
            self._disposed = True
            self._logger.info(f"Initiating disposal for Worker '{self.id}'.")
            ...
            if self._signal_controller:
                try:
                    ...
                    if hasattr(self, '_logger') and self._logger:
                        self._logger.warning(
                            f"Error unregistering Worker '{self.id}' from SignalController: {e}",
                            exc_info=True
                        )
                finally:
            ...
            if hasattr(self, '_logger') and self._logger: # Check before using/nullifying
                self._logger.info(f"Worker '{self.id}' disposal complete.")
            self._logger = None
...
#region Signal Controller Integration
    def set_external_controller(self, controller: 'SignalController'):
        ...
        This allows the Worker to register with a controller even after construction,
    ...
    def id(self) -> str:
        """
        The unique identifier for this Worker instance, conforming to SignalController's contract.
        """
    ...
    def _get_object_details(self) -> ConcurrentDict[str, Any]:
        ...
        """
        # Define commands that you want the SignalController to be able to invoke on this Worker.
        # For example, commands to stop, hard_kill, or get its status.
        return ConcurrentDict({
            "name": self.__class__.__name__,  # Or a more descriptive name like "WorkerThread"
    ...
    def set_worker_state(self, state_str: str):
        ...
        Args:
            state_str: The string representation of the desired WorkerState.

        Raises:
            ValueError: If the provided string does not match any WorkerState enum member.
        """
        with self._lock:
            try:
                new_state = WorkerState[state_str.upper()]
                if self.state != new_state:
                    self._logger.info(
                        f"Worker '{self.id}' state changed from {self.state.name} to {new_state.name}.")
                    self.state = new_state
                    self._notify("WORKER_STATE_CHANGED", {"state": self.state.name})
                else:
                    self._logger.debug(f"Worker '{self.id}' state is already {new_state.name}.")
            except KeyError:
                ...
                logger_to_use.error(
                    f"Invalid worker state string: '{state_str}'. Must be one of {[s.name for s in WorkerState]}.")
                raise ValueError(
                    f"Invalid worker state string: '{state_str}'. Must be one of {[s.name for s in WorkerState]}.")
    ...
    def _notify(self, event_type: str, data: Optional[dict] = None):
        ...
        This is a lightweight way for the Worker to report state or activity
    ...
    def get_state(self) -> WorkerState:
        ...
        Returns:
            WorkerState: The current state of the worker.
        """
        with self._lock:
            return self.state
    ...
    def hard_kill(self) -> None:
        ...
        After calling `hard_kill`, the worker's state is immediately set to `WorkerState.KILLED`.
        ...
        self._logger.warning(f"Worker '{self.id}' forcefully terminated (hard_kill).")
